...
import zabmtri.entity.EArInvPmtDisc;
...
public class SalesExporter extends BaseExport {
	...
	private void exportPreInvoice(Node transaction) {
		for (EArInv data : getPreInvoiceList()) {
			Node trans = doc.createElement("DELIVERYORDER");
			...
			for (EArInvDet det : data.detail) {
				...
				transDetail.appendChild(node("SOSEQ", Util.avoidZero(det.soseq)));
	...
	private void exportArPmt(Node transaction) {
		for (EArPmt data : getPaymentList()) {
			...
			for (EArInvPmt det : data.detail) {
				...
				for (EArInvPmtDisc disc : det.writeoff) {
	...
	protected List<EArInv> getPreInvoiceList() {
		return AppData.arDel;
	}
